...
// --- Ruta Principal (para UptimeRobot y verificar que está en línea) ---
app.get("/", (req, res) => {
  console.log("Visita de UptimeRobot recibida. El bot está despierto.");
  res.status(200).send("¡El servidor del bot de Instagram está en línea y despierto!");
});

// --- RUTA DE VERIFICACIÓN DEL WEBHOOK (GET) ---
app.get("/webhook", (req, res) => {
  console.log("Recibida petición de verificación de webhook de Meta...");
  ...
  if (mode && token) {
    if (mode === "subscribe" && token === VERIFY_TOKEN) {
      ...
      console.log("Fallo en la verificación. Los Tokens no coinciden.");
      res.sendStatus(403);
    }
  } else {
    res.sendStatus(400);
...
async function getGeminiResponse(prompt) {
  if (!GEMINI_API_KEY) {
      ...
      return "Lo siento, no puedo conectarme a mi cerebro en este momento. El administrador necesita revisar la configuración.";
  }
  ...
  try {
    ...
    return "Ups, algo salió mal al procesar tu solicitud. Inténtalo de nuevo.";
  }
}
...
async function sendInstagramMessage(recipientId, text) {
  ...
  const url = `https://graph.facebook.com/v19.0/me/messages?access_token=${MESSENGER_ACCESS_TOKEN}`;
  ...
  try {
    ...
    if (!response.ok) {
        const errorBody = await response.json();
        console.error("Error de la API de Messenger:", errorBody);
        throw new Error(`Error de API de Messenger: ${response.status}`);
    }
